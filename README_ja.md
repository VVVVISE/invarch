...
このプロジェクトは現在 「LEGO 」としているが、商標上の配慮から近々 「~~INVARCH~~ ♻️ これも商標権の懸念が判明したので再検討中 」と改名する予定だ。私はこのオープンソースプロジェクトに取り組んでいる唯一の開発者でなのです。開発が滞りなく進めば、2025年後半にはベータ版、2027年前半にはアルファ版のリリースを見込んで [マイルストーン](https://github.com/VVVVISE/lego/milestones?direction=asc&sort=due_date&state=open) を引いた。
